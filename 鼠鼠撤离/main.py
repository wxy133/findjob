...
# ------------------------------------------------------------------
# 资源路径兼容：支持直接运行 & PyInstaller 单文件解压后运行
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 配色板（来源：仓鼠跑路图片的暖米+木门棕+领带黑+仓鼠橙黄毛）
# ------------------------------------------------------------------
PALETTE = {
    "bg_cream":       "#FBF5EC",   # 主背景 - 奶油米
    "bg_card":        "#FFF9EF",   # 卡片背景 - 暖奶白
    "bg_card_alt":    "#FBEFDC",   # 强调卡片 - 淡杏
    "bg_drop":        "#FBE5C6",   # 拖放区 - 淡杏橙
    "bg_drop_border": "#D9A96A",   # 拖放区边框
    "bg_log":         "#FBF6EC",   # 日志 - 纸米

    "fg_main":        "#4A331A",   # 正文 - 深棕
    "fg_sub":         "#7B5C3C",   # 次要文字
    "fg_hint":        "#A08565",   # 提示文字
    "fg_white":       "#FFFDF9",   # 反白

    "primary":        "#C47530",   # 主按钮 - 暖橙棕（木门把手色）
    "primary_dark":   "#A45E22",   # 主按钮按下
    "primary_hover":  "#D58846",
    "success":        "#6E8A3E",   # 成功 - 橄榄暖绿
    "warning":        "#B8852B",   # 警告 - 蜂蜜棕
    "danger":         "#B6413B",   # 错误 - 棕红（门上裂纹色）
    "info":           "#8A6A45",   # 普通信息 - 棕褐
    "accent_amber":   "#E7B668",   # 点缀 - 仓鼠毛色
    "divider":        "#E4CFA9",   # 分隔线
}
...
class JobApp:
    def __init__(self, root):
        self.root = root
        self.root.title("鼠鼠撤离 · BOSS直聘自动投递")
        self.root.geometry("1120x800")
        self.root.minsize(980, 720)
        self.root.configure(bg=PALETTE["bg_cream"])

        self._load_assets()           # 加载仓鼠素材（横幅/圆形头像/窗口图标）
    ...
    def _set_window_icon(self):
        """将仓鼠ICO设为exe/窗口的任务栏图标。优先ICO，退化为PNG。"""
        if not PIL_AVAILABLE:
            return
        for cand in [_res_path("assets/app.ico"),
            ...
            try:
                if cand.lower().endswith(".ico"):
                    ...
                    self._window_icon_ref = tkimg  # 防GC
                break
            except Exception as e:
                print("设置窗口图标失败:", e)
    ...
    def _setup_style(self):
        ...
        style.map('Secondary.TButton',
                  background=[('active', '#F6E0BC')])

        # Entry / Combobox / Spinbox 暖色基调
    ...
    def _build_ui(self):
        ...
        # The canvas window must follow the viewport width; otherwise parsed
        # text can expand the left panel beyond the visible scroll area.
    ...
    def _build_header_banner(self):
        ...
        if self._header_tk:
            ...
            tk.Label(title_frame,
                     text="不干了！提起小桶去投新简历  🐹  简历解析 → BOSS直聘自动投递",
                     font=('Microsoft YaHei UI', 10),
                     fg=PALETTE["fg_sub"],
            ...
            ttk.Label(header, text="🐹  鼠鼠撤离", style='Title.TLabel').pack(side=tk.LEFT)
    ...
    def _build_resume_section(self, parent):
        ...
        # Each parsed field owns its value label. The former code created all
        # values in info_frame and only appeared to put them in their rows,
        # which makes Tkinter lay out longer parsed values out of order.
    ...
    def _update_resume_info(self, data):
        ...
        if education and isinstance(education[0], dict):
            ...
            # Graceful support for old parser output while users update.
    ...
    def _start_apply(self):
        ...
        self._append_log(f"🐹 仓鼠提起了小水桶，冲鸭！开始为「{keyword}」在 {city} 自动投递...")
