...
echo.
echo [3/6] 生成猫猫素材 (ICO / 圆形头像 / 横幅)...
python tools\make_kitty_from_original.py
if errorlevel 1 (
    echo ❌ 素材生成失败
    pause
    exit /b 1
...
if exist "猫猫跑路.spec" del /q "猫猫跑路.spec"

echo.
echo [5/6] 开始打包单文件 EXE（含猫猫图标），请耐心等待...
echo.
...
echo ================================================================
echo    ✅ 打包完成！
echo    可执行文件: dist\猫猫跑路.exe
echo    桌面图标: 已使用猫猫 ICO（16/24/32/48/64/128/256 多尺寸）
echo ================================================================
echo.
echo 💡 使用提示:
